...
import com.io.sdchain.mvp.presenter.BindEmailPresenter;
...
@Route(path = ARouterPath.BindEmailActivity)
public final class BindEmailActivity extends BaseActivity<BindEmailPresenter> implements BindEmailView {
    ...
    private void initView() {
        ...
        title.setText(getString(R.string.info75));
    ...
    private void initData() {
        ...
        oldEmail.setText(userBean.getEmail());
    ...
    public void onClickView(View view) {
        switch (view.getId()) {
                ...
                if (canClick) {
                    if (CheckFormat.checkEmail(getEditString(email))) {
                        if (getEditString(email).equals(userBean.getEmail())) {
                            showToast(getString(R.string.info76));
                            return;
                        }
                        ...
                        showToast(getString(R.string.info77));
                    }
                }
                break;
            case R.id.emailBindBtn:
                ...
                if (codeInfo == null) {
                    showToast(getString(R.string.info78));
                    return;
                }
                if (!CheckFormat.isEmail(getEditString(email))) {
                    showToast(getString(R.string.info79));
                    return;
                }

                if (getEditString(email).equals(userBean.getEmail())) {
                    showToast(getString(R.string.info80));
                    return;
                }

                UserBean userInfo = (UserBean) tools.getObjectData(Constants.USERINFO);
                getPresenter().bindEmail(
                        codeInfo.getSmsId(),
                        userInfo.getId(),
                        getEditString(code),
    ...
    public void updateTimeOver() {
        ...
        timerDown.setText(getString(R.string.info81));
        canClick = true;
    }

    @Override
    public void onCodeSuccess(Object data, String msg) {
        showToast(getString(R.string.info82));
